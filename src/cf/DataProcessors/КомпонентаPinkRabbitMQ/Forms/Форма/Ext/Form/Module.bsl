...
&НаКлиенте
Процедура ПолучитьJsomИзСправочника(Команда)
	
	ПолучитьJsomИзСправочникаНаСервере();

КонецПроцедуры

&НаСервере
Процедура ПолучитьJsomИзСправочникаНаСервере()
	
    ТекстСообщения = СсылкаНаДокумент.Сообщение;	
...
&НаКлиенте
Процедура СоздатьЭлементСправочника2(Команда)
	СоздатьЭлементСправочника2НаСервере();
КонецПроцедуры

&НаСервере
Процедура СоздатьЭлементСправочника2НаСервере()
	
	Чтение = Новый ЧтениеJSON;
	Чтение.УстановитьСтроку(ОтветноеСообщение);
	Данные = СериализаторXDTO.ПрочитатьJSON(Чтение, Тип("ДокументОбъект.РеализацияТоваровИУслуг"));
	Чтение.Закрыть();
	
	Данные.Записать();
	
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = "Создан элемент " + Данные;
	Сообщение.Сообщить();
